...
- **默认本地状态根目录**：新安装默认使用 `~/.computer-use/` 保存配置、日志、截图、trace 和业务 task；旧 `~/.kimi-code/mcp/computer-use/config.yaml` 仅作为兼容 fallback。
...
6. **优先使用 MCP 原生工具调用，不要用 Bash 写 Python 再调 `_call_tool`**：支持 MCP 的客户端通常会直接暴露 `computer-use` 工具，模型应直接传 JSON 参数调用；Bash 里写 `python -c "import sys..."` 会浪费上下文、增加出错面，只在需要 Windows API（如读 `SysListView32`）时才用 Bash。
