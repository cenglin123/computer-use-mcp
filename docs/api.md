...
**内联图像压缩**（v2.9.0+）：当 `include_image=true` 时，内联图像会经过以下处理以降低上下文体积：
...
- 压缩后 base64 仍超 `MAX_INLINE_IMAGE_BASE64_BYTES`（3 MB）时，会退化为纯路径并返回 `inline_image_skipped`
- `image.inline.max_width` 设为 `0` 可跳过缩放（仅 JPEG 编码）；`image.inline.jpeg_quality` 取值 1-100
